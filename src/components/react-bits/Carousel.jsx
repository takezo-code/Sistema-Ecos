    ...
    <motion.div
      className={`relative shrink-0 flex flex-col ${
        round
          ...
          : 'items-start justify-between bg-[#111] border border-[#2a2a2a] rounded-[12px]'
      } overflow-hidden cursor-grab active:cursor-grabbing`}
      style={{
        width: itemWidth,
        height: round ? itemWidth : '100%',
        minHeight: round ? undefined : 280,
        rotateY,
        ...(round && { borderRadius: '50%' }),
      }}
      transition={transition}
    >
      <div className={`${round ? 'p-0 m-0' : 'mb-2 p-5 pb-0'}`}>
        <span className="flex h-[28px] w-[28px] items-center justify-center rounded-full bg-[#1a1a1a] border border-[#2a2a2a]">
          {item.icon}
        </span>
      </div>
      <div className="flex flex-1 flex-col p-5 pt-3">
        <div className="mb-2 font-black text-base text-[#e5e5e5] tracking-wide">
          {item.title}
        </div>
