...
function classifyLine(raw) {
  ...
  if (/^MANUAL DO JOGADOR/i.test(text)) return { type: 'title', text }
...
function createWriter(pdf) {
  ...
  const drawFooter = () => {
    ...
    pdf.text(`Sistema Eco — Manual do Jogador · ${page}`, MARGIN, PAGE_H - 10)
  }
...
export async function downloadPlayerManualPdf() {
  ...
  createWriter(pdf).render(buildBlocks(manualText))
